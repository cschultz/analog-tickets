...
import { getFoodVendorBySlug } from "@/data/foodVendors";
...
// Per-vendor logo size overrides
const heroLogoSizeMap: Record<string, string> = {
  'commons-pizza': 'max-h-32 md:max-h-40',
};

const FoodVendorPage = () => {
  const { slug } = useParams<{ slug: string }>();
  const vendor = slug ? getFoodVendorBySlug(slug) : undefined;
  ...
  usePageMeta({
    title: vendor ? `${vendor.name} × Cosmico 2026` : "Food Partners — Cosmico",
    description: vendor
      ? `${vendor.name} is joining Cosmico 2026. Live music, wine, food, and community in Example County.`
      : "Food partners at Cosmico 2026.",
  });
            ...
            {vendor.logo && (
              ...
              >
                <img
                  ...
                  className={`w-auto object-contain ${heroLogoSizeMap[vendor.slug] || 'max-h-20 md:max-h-24'}`}
                  style={{ opacity: 0.95 }}
                />
              </motion.div>
            )}
        ...
        <div className="relative min-h-[50vh] md:min-h-screen overflow-hidden order-1 md:order-2" style={{ backgroundColor: COLORS.charcoal }}>
          {vendor.founderPhoto && (
            <img
              ...
              alt={`${vendor.founderNames} of ${vendor.name}`}
      ...
      <section id="what-is-this" className="relative py-20 md:py-28 px-6" style={{ backgroundColor: COLORS.dustySky }}>
        ...
        <div className="relative z-10 max-w-4xl mx-auto">
          ...
          >
            <div>
              ...
              <p style={{ ...typography.body, color: COLORS.charcoal, fontSize: '0.95rem', lineHeight: 1.6 }}>
                Live music by the river. Wine poured by the people who made it. Food made by people who care. Late-night dancing. Morning swims.
              </p>
            </div>
            <div>
            ...
            <div>
              <p style={{ ...typography.caption, color: COLORS.clay, letterSpacing: '0.12em', fontSize: '10px', marginBottom: '12px' }}>THE FOOD</p>
              <p style={{ ...typography.body, color: COLORS.charcoal, fontSize: '0.95rem', lineHeight: 1.6 }}>
                Local food partners cooking on-site all weekend. No mystery vendors — just makers we love and trust.
              </p>
            </div>
          </motion.div>
          ...
          <motion.p
            ...
            variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }}
          >
            Music by the Example River… wine poured by the people who made it… food cooked by people you'll meet. You won't just be there. You'll be in it.
          </motion.p>
                ...
                onSubmit={async (e) => {
                  ...
                  try {
                    await supabase.from('contact_submissions').insert({
                      email: detailsEmail,
                      name: `Food Partner Interest – ${vendor.name}`,
                      message: `Details request from ${vendor.slug} food vendor landing page`,
                    });
        ...
        <div className="relative z-10 max-w-xl mx-auto text-center">
          ...
          <motion.div className="max-w-sm mx-auto" variants={fadeInUp} initial="hidden" whileInView="visible" viewport={{ once: true }}>
            <p style={{ ...typography.body, color: COLORS.white, marginBottom: '24px', fontSize: '0.95rem', opacity: 0.85, lineHeight: 1.55 }}>
              {vendor.servingWithNames || vendor.founderNames} will be there cooking. We'd love you to be there too.
            </p>
